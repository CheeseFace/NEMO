...
CONTAINS
   ...
   SUBROUTINE icb_thm( kt )
      ...
      DO WHILE( associated(this) )
         ...
         !+ seg
         &                 pt%sst, pt%sss, pt%cn, pt%hi, zff )
         !&                 pt%sst, pt%cn, pt%hi, zff )
         !- seg
         !
         zSST = pt%sst
         !+ seg
         ! Steve George 29.01.26
         ! NEMO 4.2 backport
         zSSS = pt%sss
       ...
       ! F   = zT - D ! freeboard
         ...
         !+ seg 
         ! Steve George 29.0.26 Backport from NEMO 4.2.0 
         !  Basal melting only if above freezing
         ...
         IF ( zSST > zfzpt ) THEN                                                              ! Calculate basal melting only if SST above freezing point  
            ! Note: original backport uses basal temps and velocities
            !  zMb = MAX( 0.58_wp*(zdvob**0.8_wp)*(ztb+4.0_wp)/(zL**0.2_wp) , 0._wp ) * z1_rday
            ! Kept old version
            zMb = MAX( 0.58*(zdvo**0.8)*(zSST+4.0)/(zL**0.2)      , 0._wp ) * z1_rday   
         ELSE
