...
CONTAINS
   ...
   SUBROUTINE icb_utl_copy()
      ...
      !+seg
      ss_e(:,:) = 0._wp ;   ss_e(1:jpi, 1:jpj) = sss_m(:,:)
      !-seg
      ...
      !+seg
      CALL lbc_lnk_icb( ss_e, 'T', +1._wp, 1, 1 )
      !-seg
#if defined key_lim2
   ...
   SUBROUTINE icb_utl_interp( pi, pe1, puo, pui, pua, pssh_i,   &
      ...
      !+seg
      ! Steve George 29.01.26 NEMO 4.2 backport
      !REAL(wp), INTENT(  out) ::   psst, pcn, phi, pff            ! SST, ice concentration, ice thickness, Coriolis
      REAL(wp), INTENT(  out) ::   psst, psss, pcn, phi, pff            ! SST, ice concentration, ice thickness, Coriolis
      !-seg
      !
      ...
      !+seg
      ! Interpolated on T-grid
      psss = icb_utl_bilin_h( ss_e, pi, pj, 'T' )             ! SSS
      !-seg
